...
def read_data():
    # 读取csv文件
    raw = pd.read_csv('psosearch_data.csv').dropna().astype('float64').values
    features = raw[:, :-1]
    targets = raw[:, -1]
...
def create_and_fit_model(x_train, y_train):
    ...
    #                         param_grid={'svr__gamma':[0.05],'svr__C':[200]}, cv=10)
    # model_pz =SVR(gamma=0.05,C=200)
    # model_pz = GridSearchCV(make_pipeline(StandardScaler(),GradientBoostingRegressor(n_estimators=300, learning_rate=0.01, max_depth=2,random_state=33)),
...
def calc_func(x):
    ...
    search_range = [15, 16, 17, 18,19,20]
    ...
    # data_template=[[930, 0, 0, 340.2, 70.2, 60, 120, 0.5, 0.26, 0.65, 0.015, 0.024, 0.01, 0.15, 930, 30, 895.812, 200, 0.02, 0.21, 0.91]]
    data_template = [[930, 0, 0, 540, 15, 140, 120, 0.5, 0.35, 0.51, 0.008, 0.012, 0.11, 0.18, 930, 30, 850, 160, 1.69, 0.23, 0.55]]
    # data_template=[[1,0,0,1,0.21,1,1,0.02,0.10,0.11,0.21,0.33,0.40,0.75,1,0,0.94,0.2,0.61,0.15,0.47]]
...
def calc_pso():
    ...
    pso = PSO(func=calc_func, dim=num, pop=300, max_iter=1000,
